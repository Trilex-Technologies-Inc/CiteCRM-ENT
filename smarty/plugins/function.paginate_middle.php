...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    if(isset($_page_limit)) {
	 ...
	 if($params['format'] == 'select') {
		...
		$_this_url .= SmartyPaginate::getUrlVar($_id) . '=';
		$_ret = "<select onchange=\"document.location='" . $_this_url . "' + this.options[this.selectedIndex].value;\">";
	}
   ...
   while($_item <= $_total) {
        if($params['format'] == 'page') {
            ...
            $_text .= $_suffix;
                ...
                if($params['format'] == 'select') {
                        $_ret .= "<option value=\"" . $_item ."\" ";
                        if($_item == $_curr_item) {
                                $_ret .= " selected ";
                        }
                        $maxPages = ceil($_total / $_limit);
                        $_ret .= ">Page " . $_page . " of " . $maxPages . "</option>";
                } else {
                if($_item != $_curr_item) {
                $_this_url = $_url;
                    $_this_url .= (strpos($_url, '?') === false) ? '?' : '&';
