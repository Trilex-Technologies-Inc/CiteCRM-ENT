...
<table width="100%" cellpadding="3" cellspacing="0" border="0" class="productListing">
	<tr>
		...
		<td class="productListing-heading">Comment</td>
		<td class="productListing-heading">View</td>
	</tr>
	{foreach from=$workorder_commentArray item=workorder_comment}
	<tr>
		<td class="productListing-data">{$workorder_comment->get_workorder_comment_id()}</td>
		<td class="productListing-data">{$workorder_comment->get_workorder_id()}</td>
		<td class="productListing-data">{$workorder_comment->get_workorder_comment_create_by()}</td>
		<td class="productListing-data">{$workorder_comment->get_workorder_comment_text()}</td>
		<td class="productListing-data"><a href="index.php?page=workorder_comment:view_workorder_comment&workorder_comment_id={$workorder_comment->get_workorder_comment_id()}"><img src="images/icons/srch_16.gif" alt="View" border="0"></a></td>
	</tr>
	{foreachelse}
	<tr>
