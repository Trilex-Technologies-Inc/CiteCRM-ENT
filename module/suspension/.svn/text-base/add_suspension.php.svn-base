...
* Type:     Cite CMS PHP Page<br>
	* Name:     add_suspension.php<br>
	* Purpose:  Add New Suspension<br>
	* 
...
if(isset($_REQUEST['submit']) ) {
	...
	SmartyValidate::connect($smarty);
		...
		if(SmartyValidate::is_valid($_POST)) {
			...
			$suspension_id = $suspension->add_suspension($_REQUEST);
			$core->force_page("index.php?page=suspension:view_suspension&suspension_id=".$suspension_id);
		} else {
			// error, redraw the form
			$smarty->assign($_POST);
			$smarty->display('suspension/add_suspension_frm.tpl');
		}
} else {
	// Display the Form
	SmartyValidate::connect($smarty, true);
	SmartyValidate::register_form('new_suspension_frm');
